...
public class SpatialAnchorsExample : MonoBehaviour
{
    ...
    private const string CornersPrefsKey = "TableCornerIds";
    private List<ARAnchor> tableCorners = new();
    private List<string> savedCornerIds = new();
    private Dictionary<string, ARAnchor> recoveredCorners = new();
    ...
    private IEnumerator Start()
    {
        ...
        storageFeature.OnPublishComplete += OnCornerPublished;

        anchorManager.anchorsChanged += OnAnchorsChanged;

        string storedCornerIds = PlayerPrefs.GetString(CornersPrefsKey, string.Empty);
        if (!string.IsNullOrEmpty(storedCornerIds))
        {
            savedCornerIds = storedCornerIds.Split(',').Where(id => !string.IsNullOrEmpty(id)).ToList();
        }
    ...
    private async void OnBumper(InputAction.CallbackContext _)
    {
        if (tableCorners.Count >= CornerCount)
        {
            statusText.text = $"All {CornerCount} corners already placed. Press Publish.";
            return;
        }

        if (!permissionGranted)
        {
            statusText.text = "No Space permission. Check the application permissions.";
            return;
        }

        if (localizationMapFeature == null)
        {
            statusText.text = "Localization Maps feature is not enabled.";
            return;
        }

        localizationMapFeature.GetLatestLocalizationMapData(out LocalizationEventData latestMapData);
        if (latestMapData.State != LocalizationMapState.Localized)
        {
            statusText.text = $"Localize to the map first before placing an anchor. State: {latestMapData.State}";
            return;
        }

        Pose currentPose = new Pose(controllerObject.transform.position, controllerObject.transform.rotation);

        var result = await anchorManager.TryAddAnchorAsync(currentPose);

        if (!result.status.IsSuccess() || result.value == null)
        {
            statusText.text = $"Failed to add anchor: {result.status.statusCode} ({result.status.nativeStatusCode})";
            return;
        }

        ARAnchor newAnchorComponent = result.value;

        MeshRenderer mr = newAnchorComponent.GetComponentInChildren<MeshRenderer>();
        if (mr == null && anchorPrefab != null)
        {
            GameObject visual = Instantiate(anchorPrefab, newAnchorComponent.transform, false);
        ...
        tableCorners.Add(newAnchorComponent);

        if (tableCorners.Count == CornerCount)
        {
            surfaceReady = true;
            statusText.text = $"{CornerCount}/{CornerCount} corners placed. Press Publish.";
        }
        else
        {
            statusText.text = $"Corners: {tableCorners.Count}/{CornerCount}";
        }
    }
    ...
    private void OnMenu(InputAction.CallbackContext _)
    {
        ...
        {
            ARAnchor removed = localAnchors[^1];
            tableCorners.Remove(removed);
    ...
    public void PublishAnchors()
    {
        ...
        // Reset the saved corner group before collecting the new publish results.
        savedCornerIds.Clear();
        storageFeature.PublishSpatialAnchorsToStorage(pendingPublish, 0);
    }

    private void OnCornerPublished(ulong anchorId, string anchorMapPositionId)
    {
        if (string.IsNullOrEmpty(anchorMapPositionId))
        {
            return;
        ...
        savedCornerIds.Add(anchorMapPositionId);

        if (savedCornerIds.Count == CornerCount)
        {
            PlayerPrefs.SetString(CornersPrefsKey, string.Join(",", savedCornerIds));
            PlayerPrefs.Save();
            statusText.text = "Corners saved.";
        }
    }
    ...
    private void OnAnchorsChanged(ARAnchorsChangedEventArgs anchorsChanged)
    {
        ...
        foreach (ARAnchor anchor in anchorsChanged.added)
        {
            if (activeSubsystem.IsStoredAnchor(anchor))
            {
                ...
                TryRegisterRecoveredCorner(anchor);
            }
        }
        ...
        foreach (ARAnchor anchor in anchorsChanged.updated)
        {
            ...
            {
                MeshRenderer publishedRenderer = anchor.GetComponentInChildren<MeshRenderer>();
                if (publishedRenderer != null)
                {
                    publishedRenderer.material.color = Color.white;
                }
                storedAnchors.Add(anchor);
        ...
        foreach (ARAnchor anchor in anchorsChanged.removed)
        {
            ...
            if (tableCorners.Contains(anchor))
            {
                tableCorners.Remove(anchor);
                surfaceReady = false;
            }

            string removedId = activeSubsystem.GetAnchorMapPositionId(anchor);
            if (!string.IsNullOrEmpty(removedId))
            {
                recoveredCorners.Remove(removedId);
            }
        }
    }

    private void TryRegisterRecoveredCorner(ARAnchor anchor)
    {
        string anchorMapPositionId = activeSubsystem.GetAnchorMapPositionId(anchor);
        if (string.IsNullOrEmpty(anchorMapPositionId) || !savedCornerIds.Contains(anchorMapPositionId))
        {
            return;
        }

        recoveredCorners[anchorMapPositionId] = anchor;

        if (!tableCorners.Contains(anchor))
        {
            tableCorners.Add(anchor);
        }

        if (recoveredCorners.Count >= CornerCount && tableCorners.Count >= CornerCount)
        {
            surfaceReady = true;
            statusText.text = "Table surface ready.";
        }
    }

    /// <summary>
    /// Dört köşe anchor'ı da mevcut ve yüzey kullanılabilir durumda mı?
    /// DefectSurfaceVisualizer gibi dış tüketiciler için salt-okunur erişim.
    /// </summary>
    public bool SurfaceReady
    {
        get { return surfaceReady && tableCorners.Count == CornerCount; }
    }

    /// <summary>
    /// Köşeleri (u,v) eşlemesi için deterministik sırada döndürür: köşeler
    /// yayınlanmış ve kimlikleri biliniyorsa publish sırası (savedCornerIds),
    /// aksi halde yerleştirme sırası (tableCorners) kullanılır.
    /// Beklenen yerleştirme sırası: 1=görüntü sol-üst, 2=sağ-üst, 3=sağ-alt, 4=sol-alt.
    /// Yüzey hazır değilse null döner.
    /// </summary>
    public List<ARAnchor> GetOrderedCornersForMapping()
    {
        if (tableCorners.Count < CornerCount)
            return null;

        if (activeSubsystem != null && savedCornerIds.Count == CornerCount)
        {
            List<ARAnchor> ordered = new List<ARAnchor>(CornerCount);
            foreach (string cornerId in savedCornerIds)
            {
                ARAnchor match = tableCorners.Find(a =>
                    a != null && activeSubsystem.GetAnchorMapPositionId(a) == cornerId);
                if (match == null)
                {
                    ordered = null;
                    break;
        ...
        return new List<ARAnchor>(tableCorners);
    }
